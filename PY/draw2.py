...
class Draw(object):
    ...
    def meta_scan(self):
        ...
        # cherche pour le prochain point
        self.scan(1)

        # augmente le rayon de recherche pour trouver un
        # point proche où reprendre.
        # for i in [1,2,3,4,5,6]:
        for i in range(10000):
            if self.scan():
                return 1

            # reprend la liste du debut
            else:
    ...
    def scan(self, radius=1):
        n = self.idx - radius * self.grid.width - radius

        for y in range(-radius, radius+1):
            if self.lst[self.x - radius + self.grid.width * (self.y + y)]:
                self.next = n
                return 1

        for x in range(-radius+1, radius):
            if self.lst[self.x + x + self.grid.width * (self.y - radius)]:
                self.next = n
                return 1
            if self.lst[self.x + x + self.grid.width * (self.y + radius)]:
                self.next = n
