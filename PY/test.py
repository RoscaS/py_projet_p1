...
class Draw(object):
    ...
    def move_up(self):
        ...
        print('self.next(idx):', self.idx_to_xy(self.next))
        print(self.x, self.y)
        ...
        if self.j != dy:
            ...
            if dy >0:
                ...
                print('else self.j != dy')
                self.y -= 1
                self.j -= 1

        # condition de sortie ...
        if self.i == dx and self.j ==dy:
            print('if self.i == dx and self.j ==dy:')
            sleep(0.2)
            self.pen = 1
            self.i, self.j = 0, 0
            self.idx = self.next
            self.x, self.y = self.idx_to_xy(self.next)
            self.move() # back to move()
        ...
        # ... else recursion
        print('point recursion')
        self.window.after(self.speed, self.move_up)
